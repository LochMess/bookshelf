...
describe('smoke', () => {
  it('should allow a typical user flow', () => {
    ...
    cy.findByRole('main').within(() => {
      cy.findByAllRole('listitem').should('have.length', 1)
      cy.findByRole('listitem', {name: new RegExp(bookTitle, 'i')}).click()
    })
    ...
    cy.findByRole('textbox', {name: /notes/i}).type('Great book!')
    ...
    cy.findByRole('main').within(() => {
      ...
      cy.findAllByRole('textbox', {name: /notes/i}).should('not.exist')
